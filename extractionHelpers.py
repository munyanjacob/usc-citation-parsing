...
reUSC = r"((?:U\.? ?S\.? ?(?:C\.?|Code)|United States Code)" + "(?: App| ?A\.?)?)"
...
def process(s):
    ...
    # this can find all titles and USC versions within mini citation!!
    #print(len(re.findall(reTitleAndUSC, s)))
    #print(re.findall(reTitleAndUSC, s))
    ...
    # will also need to replace all group 2 (USC versions) from 'USC App' or 'U.S.C.A.' or 'USC A.' or other to USCA
    # also need to replace things like 'United States Code' with USC
