...
#	**Producing spreadsheet version 1.1**
...
totalsSheet.set_column('G:I', col_width / 8)
...
totalsSheet.write("G2", "USC/Total")
totalsSheet.write("H2", "Stat/Total")
totalsSheet.write("I2", "Both/Total")
...
for t in years:
	...
	if COUNTtotals[t] != 0:
		totalsSheet.write(row, 6, "{:.3f}".format(USCtotals[t] / float(COUNTtotals[t])))
		totalsSheet.write(row, 7, "{:.3f}".format(STATtotals[t] / float(COUNTtotals[t])))
		totalsSheet.write(row, 8, "{:.3f}".format(BOTHtotals[t] / float(COUNTtotals[t])))
